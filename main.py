    ...
    if message.content.startswith("/r") or message.content.startswith("/roll"):
        ...
        if check_err == -1:
            ...
            for result in results:
                ...
                msg += "\n----------\nTotal: {} \nBase (1d20 -> {}): \t {}".format( (total_base + total_attr), total_base, base_kept)
                if base_dropped != "-" and base_dropped != "0":  # Hides Dropped rolls if there are none.
                    msg += "\n> \t *Dropped: {}*".format(base_dropped)
                if base_dropped_vs != "-" and base_dropped_vs != "0":  # Hides Vicious Strike if there is no value associated with the dropped values.
                    msg += "\n> \t _Dropped  **(Vicious Strike)**: {}_".format(base_dropped_vs)
                
                msg += "\nAttribute ({} -> {}): \t {}".format(info, total_attr, attr_kept)
                if attr_dropped != "-" and attr_dropped != "0":  # Hides Dropped rolls if there are none.
                    msg += "\n> \t *Dropped: {}*".format(attr_dropped)
